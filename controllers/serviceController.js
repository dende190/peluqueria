...
module.exports = {
	listService: (req, res) => {
		...
		dbo.collection("services").find({}).toArray( (err, result) =>{
			if(err){
	...
	createService: (req, res) => {
		...
		if(req.body.name){
			...
			dbo.collection("services").insertOne( { ...newService }, (err, result) => {
				if(err){
	...
	editService: (req, res) => {
    	...
    	if(req.body.name){
			...
			dbo.collection('services').replaceOne( { _id: ObjectId(req.params.id) }, { ...newService } , (err, result) => {
				if(err){
			...
			dbo.collection('services').findOne( { _id: ObjectId(req.params.id) }, (err, result) => {
				if(err){
    ...
    deleteService: (req, res) => {
        ...
        dbo.collection('services').remove( {_id: ObjectId(req.body.id) }, (err, r) => {
            if(err){
